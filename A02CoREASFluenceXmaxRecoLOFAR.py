...
path = '/home/kwatanabe/Projects/radio-ift/resources/lofar/hdf5_sims/45616753/1/proton'

det = detector.Detector(json_filename="LOFAR/LOFAR.json",
...
input_file = os.path.join(path, "SIM000012.hdf5")

# instead of using this, need to use the HACK due to core coordinate issue
# coreas_reader.begin(input_file)

evt = coreas.read_CORSIKA7(input_file)
...
interpolator.initialize_efield_interpolator(interp_lowfreq=30 * units.MHz, interp_highfreq=1000 * units.MHz)
coreas_reader.coreas_interpolator = interpolator  # skip begin() function because HDF5 does not have good CoreCoordinateVertical
coreas_reader._readCoREASDetector__corsika_evt = evt



# we only need a single realization of the shower, so we set the core position to zero for simplicity
for iE, evt in enumerate(coreas_reader.run(det, [core], selected_station_channel_ids=selected_station_channel_ids)):
    ...
    efieldToVoltageConverter.run(evt, station, det, channel_ids=selected_station_channel_ids[station.get_id()])
    ...
    if station.get_trigger('default_simple_threshold').has_triggered():
        ...
        for i, filename2 in enumerate(input_files):
            ...
            evt2.get_first_sim_shower().set_parameter(
                shp.core, np.array([0,0,evt.get_first_sim_shower().get_parameter(shp.observation_level)])
            )
