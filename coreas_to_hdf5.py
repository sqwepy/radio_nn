...
def convert_obs_positions_to_nuradio_on_ground(observer_pos, declination=0):
    ...
    obs_positions = rdhelp.rotate_vector_in_2d(obs_positions, -declination).T
...
def write_vxB_vxvxB_data(f_h5,declination):
    ...
    antennas_data = f_h5['highlevel/obsplane_na_na_vB_vvB/antenna_position_vBvvB']
    ...
    print(f'ANTENNAS: {np.array(shower_antennas).shape}')
    print(f'TRACES: {np.array(all_efield).shape}')
    print(f'TIME: {np.array(all_efield_time).shape}')
    data = np.array([shower_antennas,all_efield_time,all_efield]) 
    
        
    if "LOFAR_vxvxB" in highlevel:
        del highlevel["LOFAR_vxvxB"]
    data_set = highlevel.create_dataset("LOFAR_vxvxB", shape = data.shape, dtype=float)
    data_set[...] = data
    data_set.attrs["comment"] = (
        '[0] = Antenna positions, [1] = traces time, [2] = traces'
    )
...
#
    ...
    geomagnetic_angle = get_geomagnetic_angle(zenith, azimuth, magnetic_field_vector)
    f_h5_hl.attrs["geomagnetic_angle"] = geomagnetic_angle
    ...
    for plane in planes_unique:
        ...
        for i, j in enumerate(index[mask_plane]):
            ...
            #print(data.shape)
            #
            #efield_on_sky, efield_times = convert_obs_to_nuradio_efield(data,zenith,azimuth,magnetic_field_vector)
            #
            #print((efield_on_sky.T).shape)
            #print(efield_times.shape)
            #for l in range(0,len(efield_times)):
            #    data[i] = [efield_times[i],efield_on_sky[0][0],efield_on_sky[0][1],efield_on_sky[0][2]]
            #
            #print(data.shape)
...
#
            ...
            obs_positions = convert_obs_positions_to_nuradio_on_ground(np.array([position[0],position[1],position[2]]))
            #print(obs_positions.shape)
