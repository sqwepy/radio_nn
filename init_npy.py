...
RADIO_DATA_PATH = "/Users/denis/Desktop/BachelorThesis/memmaps/test"
...
def _init_(path_,total_amount_of_measurements = 33, parameters = 11, event_level_parameters = 13, number_of_antennas = 160,grammage_steps = 1142, time_bins = 256, dimensions_antenna_positions_vB_vvB = 3, dimensions_antenna_traces_vB_vvB = 3,dimensions_antenna_traces_ge_ce = 2,time_ge_ce_and_vB_vvB = 2):
    ...
    fp1 = np.memmap(file_name, dtype=f"{dtypeInit}", mode="w+", shape=(total_amount_of_measurements, grammage_steps, parameters))
    print(fp1.shape)
    np.save(file_name, fp1)
    file_name = os.path.join(path_, "meta_data")
    #os.remove(file_name)
    #(number of simulations, event level parameters)
    fp2 = np.memmap(file_name, dtype=f"{dtypeInit}", mode="w+", shape=(total_amount_of_measurements, event_level_parameters))
    print(fp2.shape)
    np.save(file_name, fp2)
    file_name = os.path.join(path_, "antenna_pos_data")
    #os.remove(file_name)
    #(number of simulations, number of antennas, ant positions)
    fp3 = np.memmap(file_name, dtype=f"{dtypeInit}", mode="w+", shape=(total_amount_of_measurements, number_of_antennas, dimensions_antenna_positions_vB_vvB))
    print(fp3.shape)
    ...
    fp4 = np.memmap(file_name, dtype=f"{dtypeInit}", mode="w+", shape=(total_amount_of_measurements, number_of_antennas, time_ge_ce_and_vB_vvB))
    print(fp4.shape)
    np.save(file_name, fp4)
    file_name = os.path.join(path_, "output_gece_data")
    #os.remove(file_name)
    fp5 = np.memmap(file_name, dtype=f"{dtypeInit}", mode="w+", shape=(total_amount_of_measurements, number_of_antennas, time_bins, dimensions_antenna_traces_ge_ce))
    print(fp5.shape)
    ...
    #os.remove(file_name)
    fp6 = np.memmap(file_name, dtype=f"{dtypeInit}", mode="w+", shape=(total_amount_of_measurements, number_of_antennas, time_bins, dimensions_antenna_traces_vB_vvB))
    print(fp6.shape)
    np.save(file_name, fp6)
    
if __name__ == "__main__":    #PLAYING CODE
    _init_(RADIO_DATA_PATH)
