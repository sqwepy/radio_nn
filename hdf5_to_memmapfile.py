...
def flush_input_meta(SIM_NUMBER, f_h5, index):
    ...
    d_xmax, h_xmax = density_at_X(
        f_h5, f_h5["highlevel"].attrs["gaisser_hillas_dEdX"][2]
    )
...
def flush_antenna_pos(f_h5, index):
    ...
    antenna_pos = open_memmap(antenna_pos_file, mode="r+")
...
def flush_input_data(f_h5, index):
    ...
    input_data[index, :x_len, 8] = path_along_shower
