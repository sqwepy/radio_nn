...
class AntennaDataset(Dataset):
    def __init__(
        ...
        if self.one_shower is not None:
            ...
            self.indices = np.sort(
                np.random.choice(
                    np.arange(self.total_events), size=num_samples, replace=False
                )
            )
    ...
    def __getitem__(self, idx):
        ...
        event_data = torch.log(
            torch.tensor(self.input_data[event_idx, :, 4:], dtype=torch.float32) + 1e-14
        )
        meta_data = torch.tensor(self.input_meta[event_idx], dtype=torch.float32)[1:]
        ...
        output = torch.tensor(self.output[event_idx, antenna_idx], dtype=torch.float32)
